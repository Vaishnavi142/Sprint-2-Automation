Feature: Verify Profile section of Elearning site
  ...
  Scenario Outline: verify the Forward option in Inbox
    ...
    And User select the receiver name
  ...
  Scenario Outline: Verify password change functionality
    ...
    Examples: 
      ...
      | vaish@123 | vaish@123 |           | Your new profile has been saved |

  @Linkedin
  Scenario Outline: Verify the Skype and LinkedIn URL fields
    When User click the Edit profile link
    When User enter URL "<url>" in the Skype textbox
    When User enter URL "<url1>" in the LinkedIn URL textbox
    And User clicks save settings button
    Then User is displayed with a confirmation "<text>" text

    Examples: 
      | url              | url1                      | text                            |
      | live:abcvaishjkl | www.linkedin.com/in/vaish | Your new profile has been saved |
      | $%^gkgjls        | &@$abcdxyz                | Your new profile has been saved |
